...
from app.scrapers.hp import HPHTTPScraper
from app.scrapers.epson import EpsonHTTPScraper
from app.scrapers.canon import CanonHTTPScraper
...
def _is_known_apeos_ip(ip: str) -> bool:
    try:
        ...
        return address == ipaddress.ip_address("192.168.1.200") or ipaddress.ip_address("192.168.1.10") <= address <= ipaddress.ip_address("192.168.1.50")
    except ValueError:
        return False
...
def _is_hp(printer: Printer) -> bool:
    """Return True if this printer is an HP printer."""
    if getattr(printer, "scraper_type", "") == "http_hp":
        return True
    for field in (printer.model or "", printer.manufacturer or "", printer.hostname or ""):
        if "hp" in field.lower() or "hewlett" in field.lower() or "officejet" in field.lower() or "laserjet" in field.lower():
            return True
        if field.upper().startswith("HPI") or field.upper().startswith("NPI"):
            return True
    return False

def _is_epson(printer: Printer) -> bool:
    """Return True if this printer is an Epson printer."""
    if getattr(printer, "scraper_type", "") == "http_epson":
        return True
    for field in (printer.model or "", printer.manufacturer or "", printer.hostname or ""):
        if "epson" in field.lower() or "seiko" in field.lower():
            return True
    return False

def _is_canon(printer: Printer) -> bool:
    """Return True if this printer is a Canon printer."""
    if getattr(printer, "scraper_type", "") == "http_canon":
        return True
    for field in (printer.model or "", printer.manufacturer or "", printer.hostname or ""):
...
def get_adapter(printer: Printer):
    ...
    if _is_epson(printer):
        return EpsonHTTPScraper(
            ip=printer.ip_address,
            community=printer.snmp_community or "public",
            version=printer.snmp_version or "v2c",
            timeout=3
        )
    if _is_canon(printer):
        return CanonHTTPScraper(
            ip=printer.ip_address,
            community=printer.snmp_community or "public",
            version=printer.snmp_version or "v2c",
            timeout=3
...
async def check_snmp_status():
    ...
    try:
        printers = db.query(Printer).filter(
            ...
            Printer.asset_status.notin_(["Retired", "In Storage"])
        ).all()
...
from app.services.notification import send_email_notify, get_department_email
import asyncio
        ...
        if not printer.hostname or is_fuji_default_name or not printer.location or not printer.serial_number or metadata_has_markup:
            ...
            if sys_info:
                ...
                if sys_info.get("sysLocation"):
                    printer.location = sys_info["sysLocation"]
                    metadata_changed = True
                if sys_info.get("serialNumber"):
                    printer.serial_number = sys_info["serialNumber"]
                    metadata_changed = True
                if sys_info.get("model"):
                    printer.model = sys_info["model"]
                    metadata_changed = True
...
async def _check_single_printer_supplies(printer_id: int, adapter: StandardSNMPAdapter):
    ...
    try:
        ...
        if printer:
            ...
            # --- Supplies: always write what the scraper returned ---
            # Rule: if scraper returns a value → show it. If None → hide it (clear from DB).
            # This applies to ALL scrapers (SNMP, HP web, Apeos, etc.)
            if supplies:
                # --- Toner ---
                new_toner = supplies.get("toner_level")
                prev_toner = printer.toner_level
                if new_toner != prev_toner:
                    any_changed = True
                printer.toner_level = new_toner
                printer.toner_black_level = supplies.get("toner_black_level")
                printer.toner_cyan_level = supplies.get("toner_cyan_level")
                printer.toner_magenta_level = supplies.get("toner_magenta_level")
                printer.toner_yellow_level = supplies.get("toner_yellow_level")
                # Extra Epson wide-format inks
                if hasattr(printer, "toner_photo_black_level"):
                    printer.toner_photo_black_level = supplies.get("toner_photo_black_level")
                    printer.toner_matte_black_level = supplies.get("toner_matte_black_level")
                    printer.toner_red_level = supplies.get("toner_red_level")

                if new_toner is not None:
                    from app.alerts.engine import _create_or_update_alert, _resolve_alerts
                    if new_toner <= 10:
                        msg_in_app = f"Toner is critically low ({new_toner}%)"
                        await _create_or_update_alert(db, printer.id, "SUPPLY", "CRITICAL", msg_in_app)
                        if prev_toner is None or prev_toner > 10:
                            msg = f"🟡 LOW TONER ALERT ({new_toner}%)\nName: {printer.hostname or printer.ip_address}\nIP: {printer.ip_address}\nLocation: {printer.location or '-'}"
                            dept_email = get_department_email(printer.department)
                            if dept_email:
                                asyncio.create_task(send_email_notify(f"Low Toner Alert: {printer.ip_address}", msg, dept_email, printer_ip=printer.ip_address, issue_type="low_toner"))
                    else:
                        await _resolve_alerts(db, printer.id, "SUPPLY")
                    # Auto-detect Toner Replacement
                    if prev_toner is not None and new_toner > prev_toner + 20:
                        db.add(MaintenanceLog(
                            printer_id=printer.id,
                            description=f"Auto-detected: Toner Replaced (from {prev_toner}% to {new_toner}%)",
                            performed_by="System (Auto)"
                        ))

                # --- Drum ---
                new_drum = supplies.get("drum_level")
                prev_drum = printer.drum_level
                if new_drum != prev_drum:
                    any_changed = True
                printer.drum_level = new_drum
                printer.drum_black_level = supplies.get("drum_black_level")
                printer.drum_cyan_level = supplies.get("drum_cyan_level")
                printer.drum_magenta_level = supplies.get("drum_magenta_level")
                printer.drum_yellow_level = supplies.get("drum_yellow_level")
                # Auto-detect Drum Replacement
                if new_drum is not None and prev_drum is not None and new_drum > prev_drum + 20:
                    db.add(MaintenanceLog(
                        printer_id=printer.id,
                        description=f"Auto-detected: Drum Replaced (from {prev_drum}% to {new_drum}%)",
                        performed_by="System (Auto)"
                    ))

                # --- Other consumables ---
                new_fuser = supplies.get("fuser_level")
                if new_fuser != printer.fuser_level:
                    any_changed = True
                printer.fuser_level = new_fuser

                new_laser = supplies.get("laser_unit_level")
                if new_laser != printer.laser_unit_level:
                    any_changed = True
                printer.laser_unit_level = new_laser

                new_pf_mp = supplies.get("pf_kit_mp_level")
                if new_pf_mp != printer.pf_kit_mp_level:
                    any_changed = True
                printer.pf_kit_mp_level = new_pf_mp

                new_pf1 = supplies.get("pf_kit_1_level")
                if new_pf1 != printer.pf_kit_1_level:
                    any_changed = True
                printer.pf_kit_1_level = new_pf1

            # Broadcast supply update directly only if changed
            if any_changed:
                await manager.broadcast({
                    "type": "SUPPLY_UPDATE",
                    "data": {
                        ...
                        "toner_black_level": printer.toner_black_level,
                        "toner_cyan_level": printer.toner_cyan_level,
                        "toner_magenta_level": printer.toner_magenta_level,
                        "toner_yellow_level": printer.toner_yellow_level,
                        "toner_photo_black_level": getattr(printer, "toner_photo_black_level", None),
                        "toner_matte_black_level": getattr(printer, "toner_matte_black_level", None),
                        "toner_red_level": getattr(printer, "toner_red_level", None),
                        "drum_level": printer.drum_level,
                        "drum_black_level": printer.drum_black_level,
                        "drum_cyan_level": printer.drum_cyan_level,
                        "drum_magenta_level": printer.drum_magenta_level,
                        "drum_yellow_level": printer.drum_yellow_level,
            ...
            if counters and "total_pages" in counters:
                ...
                # Find if there is already a reading for today
                existing_counter = db.query(PrinterCounters).filter(
                    PrinterCounters.printer_id == printer_id
                ).order_by(PrinterCounters.measured_at.desc()).first()
                
                if existing_counter and existing_counter.measured_at.date() == today:
                    # Update today's record to the latest reading
                    existing_counter.total_pages = counters["total_pages"]
                else:
            ...
            # Save supplies snapshot for history charting ONLY if changed
            if any_changed and supplies and any(supplies.get(k) is not None for k in ["toner_level", "drum_level"]):
...
async def check_expirations():
    """Check for upcoming warranty and lease expirations and generate alerts."""
    ...
    try:
        ...
        for p in printers:
            ...
            if p.warranty_expiry:
                if p.warranty_expiry < today:
                    msg = f"Warranty expired on {p.warranty_expiry.strftime('%Y-%m-%d')}."
                    await _create_or_update_alert(db, p.id, "WARRANTY", "CRITICAL", msg)
                elif p.warranty_expiry <= threshold:
                    days_left = (p.warranty_expiry - today).days
                    msg = f"Warranty expires in {days_left} days ({p.warranty_expiry.strftime('%Y-%m-%d')})."
                    await _create_or_update_alert(db, p.id, "WARRANTY", "WARNING", msg)
                else:
                    await _resolve_alerts(db, p.id, "WARRANTY")
            else:
                await _resolve_alerts(db, p.id, "WARRANTY")
                
            # Check Lease
            if p.lease_end_date:
                if p.lease_end_date < today:
                    msg = f"Lease ({p.lease_provider or 'Unknown Provider'}) expired on {p.lease_end_date.strftime('%Y-%m-%d')}."
                    await _create_or_update_alert(db, p.id, "LEASE", "CRITICAL", msg)
                elif p.lease_end_date <= threshold:
                    days_left = (p.lease_end_date - today).days
                    msg = f"Lease expires in {days_left} days ({p.lease_end_date.strftime('%Y-%m-%d')})."
                    await _create_or_update_alert(db, p.id, "LEASE", "WARNING", msg)
                else:
                    await _resolve_alerts(db, p.id, "LEASE")
            else:
                await _resolve_alerts(db, p.id, "LEASE")
