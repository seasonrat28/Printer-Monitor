...
@app.get("/snmp_walk")
async def snmp_walk(ip: str = "192.168.1.100", community: str = "public"):
...
async def apeos_login_debug(ip: str = "192.168.1.100", pwd: str = "admin1234"):
...
from app.models.printer import Printer, MaintenanceLog, PrinterBorrowLog, PrinterAttachment
...
async def startup_event():
    ...
    try:
        ...
        # Migration: Inventory Items
        try:
            db.execute(text("ALTER TABLE inventory_items ADD COLUMN page_yield INTEGER"))
            db.commit()
            logger.info("Added page_yield column to inventory_items table")
        except Exception:
            db.rollback()
        ...
        # Migration: borrowed_by column
        try:
            db.execute(text("ALTER TABLE printers ADD COLUMN borrowed_by VARCHAR"))
            db.commit()
            logger.info("Added borrowed_by column to printers table")
        except Exception:
            db.rollback()

        # Phase 1 Migration: Asset Management
        for col in ["asset_status VARCHAR DEFAULT 'Deployed'", "purchase_date DATE", "warranty_expiry DATE"]:
            try:
                db.execute(text(f"ALTER TABLE printers ADD COLUMN {col}"))
                db.commit()
                logger.info(f"Added {col.split()[0]} column to printers table")
            except Exception:
                db.rollback()

        # Phase 2 Migration: Lease Tracking
        for col in ["lease_provider VARCHAR", "lease_start_date DATE", "lease_end_date DATE"]:
            try:
                db.execute(text(f"ALTER TABLE printers ADD COLUMN {col}"))
                db.commit()
                logger.info(f"Added {col.split()[0]} column to printers table")
