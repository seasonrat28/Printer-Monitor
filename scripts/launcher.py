...
def get_local_ip():
    try:
        # First try to find a 10.x.x.x IP address (LAN)
        for iface_name, iface_data in socket.if_nameindex():
            pass # Just to load socket
        
        hostname = socket.gethostname()
        ip_addresses = socket.gethostbyname_ex(hostname)[2]
        
        # Prioritize 10.x.x.x network
        for ip in ip_addresses:
            if ip.startswith("10."):
                return ip
                
        # If no 10.x.x.x, use standard method
...
def _launch_browser(frontend_url):
    print_and_log(f"Opening Web Browser at: {frontend_url}")
    print_and_log("========================================")
    try:
        webbrowser.open(frontend_url)
    except Exception as e:
        print_and_log(f"Failed to open browser: {e}")
...
def start():
    ...
    if backend_running or frontend_running:
        ...
        if frontend_running and ports:
            frontend_url = ports.get('frontend_url')
            if frontend_url:
                _launch_browser(frontend_url)
        return
    ...
    print_and_log("192.168.1.0/24")
    print_and_log("192.168.2.0/24")
    print_and_log("")
...
def stop():
    ...
    e_pid = pids.get("electron_pid")
    if e_pid and check_process_running(e_pid):
        stop_process(e_pid)
        print_and_log("Desktop App stopped")
