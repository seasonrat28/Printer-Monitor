...
import { Printer as PrinterIcon, Star, RefreshCw, History, ChevronDown, ChevronUp, ExternalLink, Trash2, BarChart2, Edit2 } from 'lucide-react';
...
export const PrinterCard: React.FC<PrinterCardProps> = ({ printer, onToggleFavorite, onOpenHistory, onEdit, onDelete }) => {
    const { user } = useAuth();
    ...
    const renderCMYKBar = (type: "TONER" | "DRUM") => {
        const cLevel = type === "TONER" ? printer.toner_cyan_level : printer.drum_cyan_level;
        const mLevel = type === "TONER" ? printer.toner_magenta_level : printer.drum_magenta_level;
        const yLevel = type === "TONER" ? printer.toner_yellow_level : printer.drum_yellow_level;
        const kLevel = type === "TONER" ? printer.toner_black_level : printer.drum_black_level;
        // Epson wide-format extra inks
        const pkLevel = type === "TONER" ? (printer as any).toner_photo_black_level : undefined;
        ...
        const colors = [
            { label: 'PK', value: pkLevel, bg: 'bg-gray-900 dark:bg-gray-200', text: 'text-gray-700 dark:text-gray-300' },
            { label: 'MK', value: mkLevel, bg: 'bg-gray-600', text: 'text-gray-600 dark:text-gray-400' },
            { label: 'C',  value: cLevel,  bg: 'bg-cyan-500', text: 'text-cyan-600 dark:text-cyan-400' },
            { label: 'M',  value: mLevel,  bg: 'bg-pink-500', text: 'text-pink-600 dark:text-pink-400' },
            { label: 'Y',  value: yLevel,  bg: 'bg-yellow-400', text: 'text-yellow-600 dark:text-yellow-400' },
            { label: 'K',  value: kLevel,  bg: 'bg-gray-800 dark:bg-gray-400', text: 'text-gray-800 dark:text-gray-300' },
            { label: 'R',  value: rLevel,  bg: 'bg-red-600', text: 'text-red-600 dark:text-red-400' },
        ];

        return (
            <div className="flex-1 flex flex-col items-center w-full">
                <span className="text-[10px] uppercase font-bold text-gray-500 dark:text-gray-400 flex items-center space-x-1 tracking-wider text-center mb-1">
                    <span className="opacity-70">{type === "TONER" ? '✒️' : '🗞️'}</span>
                    <span>{type}</span>
                </span>
                <div className="flex gap-1.5 w-full mt-1 px-1 justify-center">
                    {colors.map((c, i) => (
                        c.value !== undefined && c.value !== null ? (
                            <div key={i} className="flex-1 max-w-[24px] flex flex-col items-center">
                                <span className={`text-[9px] font-bold ${c.text}`}>{c.value}%</span>
                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-sm h-6 mt-0.5 relative overflow-hidden transform rotate-180">
                                    <div
                                        style={{ height: `${c.value}%` }}
                                        className={`w-full ${c.bg} transition-all duration-1000 ease-out`}
                                    />
                                </div>
        ...
        >
            ...
            <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center space-x-2 w-full">
                        ...
                        <h3 className="font-bold text-gray-800 dark:text-gray-100 text-lg truncate flex-1 flex items-center space-x-2" title={printer.hostname || printer.ip_address}>
                            <span>{printer.hostname || printer.ip_address}</span>
                            {printer.asset_status && printer.asset_status !== 'Deployed' && (
                                <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border whitespace-nowrap
                                    ${printer.asset_status === 'In Storage' ? 'bg-blue-100 text-blue-800 border-blue-200' :
                                      printer.asset_status === 'In Repair' ? 'bg-orange-100 text-orange-800 border-orange-200' :
                                      'bg-gray-100 text-gray-800 border-gray-200'}
                                `}>
                                    {printer.asset_status}
                                </span>
                            )}
                        </h3>
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 space-y-2 mb-4 w-full">
                    ...
                    {printer.asset_status && (
                        <div className="flex justify-between border-b border-gray-100 dark:border-gray-700 pb-1">
                            <span className="font-medium text-slate-500">Asset Status:</span>
                            <span className="text-gray-800 dark:text-gray-200 text-xs font-semibold px-2 py-0.5 rounded bg-gray-100 dark:bg-gray-700">{printer.asset_status}</span>
                        </div>
                    )}
                    {printer.warranty_expiry && (
                        <div className="flex justify-between border-b border-gray-100 dark:border-gray-700 pb-1">
                            <span className="font-medium text-slate-500">Warranty:</span>
                            <span className={`text-xs font-medium ${new Date(printer.warranty_expiry) < new Date() ? 'text-red-500' : 'text-emerald-500'}`}>
                                {new Date(printer.warranty_expiry).toLocaleDateString('en-GB')}
                            </span>
                        </div>
                    )}
                </div>
                ...
                {/* Progress Bars */}
                {(() => {
                    const hasToner = printer.toner_level !== null && printer.toner_level !== undefined ||
                        printer.toner_cyan_level !== null && printer.toner_cyan_level !== undefined ||
                        printer.toner_magenta_level !== null && printer.toner_magenta_level !== undefined ||
                        printer.toner_yellow_level !== null && printer.toner_yellow_level !== undefined ||
                        printer.toner_black_level !== null && printer.toner_black_level !== undefined;
                    const hasDrum = printer.drum_level !== null && printer.drum_level !== undefined ||
                        printer.drum_cyan_level !== null && printer.drum_cyan_level !== undefined ||
                        printer.drum_magenta_level !== null && printer.drum_magenta_level !== undefined ||
                        printer.drum_yellow_level !== null && printer.drum_yellow_level !== undefined ||
                        printer.drum_black_level !== null && printer.drum_black_level !== undefined;
                    if (!hasToner && !hasDrum) return null;
                    return (
                        <div className="flex items-center justify-between space-x-6 px-2 mb-4">
                            {hasToner && renderCMYKBar("TONER")}
                            {hasToner && hasDrum && <div className="h-10 w-px bg-gray-200 dark:bg-gray-600"></div>}
                            {hasDrum && renderCMYKBar("DRUM")}
                        </div>
                    );
                })()}

                {[
                    { label: 'FUSER', value: printer.fuser_level },
                    { label: 'LASER', value: printer.laser_unit_level },
                    { label: 'PF KIT MP', value: printer.pf_kit_mp_level },
            ...
            <div className="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 dark:bg-gray-800/90 rounded-md shadow-sm p-1">
                ...
                {user?.role === 'ADMIN' && onEdit && (
                    <button onClick={() => onEdit(printer)} className="p-1.5 text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/40 rounded transition-colors" title="Edit Asset Info">
                        <Edit2 size={14} />
                    </button>
                )}
                {user?.role === 'ADMIN' && (
