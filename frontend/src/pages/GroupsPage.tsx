...
import { Users, Plus, Edit2, Trash2, Printer as PrinterIcon } from 'lucide-react';
...
export const GroupsPage = () => {
  ...
  const [managingPrintersGroup, setManagingPrintersGroup] = useState<Group | null>(null);
  ...
  return (
    <div className="space-y-6">
                ...
                <tr key={group.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end space-x-2">
                      <button onClick={() => setManagingPrintersGroup(group)} className="p-2 text-gray-400 hover:text-indigo-500 transition-colors" title="Manage Printers">
                        <PrinterIcon size={18} />
                      </button>
                      <button onClick={() => openModal(group)} className="p-2 text-gray-400 hover:text-blue-500 transition-colors" title="Edit Group">
                        <Edit2 size={18} />
                      </button>
                      <button onClick={() => handleDelete(group.id)} className="p-2 text-gray-400 hover:text-red-500 transition-colors" title="Delete Group">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </td>
      ...
      {managingPrintersGroup && (
        <ManagePrintersModal
          isOpen={true}
          onClose={() => setManagingPrintersGroup(null)}
          entityType="group"
          entityId={managingPrintersGroup.id}
          entityName={managingPrintersGroup.name}
          initialPrinterIds={managingPrintersGroup.printers?.map(p => p.id) || []}
          onSaved={() => {
            fetchGroups();
          }}
        />
