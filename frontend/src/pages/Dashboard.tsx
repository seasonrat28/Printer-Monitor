...
import { Printer, AlertTriangle, CheckCircle2, XCircle, ChevronRight, Clock, Award, BarChart3, Bell, RefreshCw, Package } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const fetchSummary = useCallback(async () => {
        try {
            const [res, invRes] = await Promise.all([
                printerService.getDashboardSummary(),
    ...
    const inventorySummary = useMemo(() => {
        const groups = inventoryItems.reduce((acc: Record<string, any>, item: any) => {
            if (!acc[item.item_code]) {
                acc[item.item_code] = { code: item.item_code, name: item.name, total: 0 };
            }
            acc[item.item_code].total += (item.quantity_total - item.quantity_used);
            return acc;
        }, {});
        return Object.values(groups) as {code: string, name: string, total: number}[];
    }, [inventoryItems]);
    ...
    return (
        <div className="space-y-6">
            ...
            {inventorySummary.length > 0 && (
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200/60 dark:border-slate-700/60 overflow-hidden mt-2">
                    <div className="px-5 py-3.5 border-b border-slate-200/60 dark:border-slate-700/60 bg-slate-50/50 dark:bg-slate-800/80 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <div className="w-1.5 h-4 bg-indigo-500 rounded-full"></div>
                            <h3 className="font-semibold text-sm text-slate-800 dark:text-slate-200 uppercase tracking-wide">สรุปยอดอะไหล่คงเหลือ (Inventory)</h3>
                        </div>
                        <Link to="/inventory" className="text-xs font-bold text-indigo-600 hover:text-indigo-500 flex items-center gap-1">
                            จัดการคลัง <ChevronRight size={14} />
                        </Link>
                    </div>
                    <div className="p-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {inventorySummary.map(sum => (
